...
int robot_peut_avancer(Terrain *t, Robot *r) {
  ...
  // Vérifier si le robot est au bord du terrain
  if (x_devant < 0 || x_devant >= t->largeur || y_devant < 0 || y_devant >= t->hauteur) {
    return 0; // Le robot est au bord, il ne peut pas avancer
  }

  // Vérifier si la case devant le robot est libre
  return t->tab[x_devant][y_devant] == LIBRE;
}
