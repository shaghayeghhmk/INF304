...
void generation_aleatoire(Terrain *T, int l, int h, float dObst) {
  float nb_aleatoire;
  int i,j;
  int obst = dObst * 100 ; // remettre dObst entre 0 et 100
  ...
  int cx = l / 2;
  int cy = h / 2;
  T->tab[cx][cy] = LIBRE; // case centrale est libre
  
  // On utilise time(NULL) pour initialiser le générateur de manière aléatoire
  srand(time(NULL));

  for (i=0; i<l; i++){ // parcourir les lignes
    for (j=0; j<h; j++){ // parcourir les collonnes
      if (i != cx || j != cy ){
        nb_aleatoire = rand()%101; // générer un nombre réel aléatoire compris entre 0 et 100
        if (nb_aleatoire < obst) {
          T->tab[i][j] = LIBRE;
        } else if (nb_aleatoire <= obst + 10) { // 10% de chance d'eau autour des obstacles
          T->tab[i][j] = EAU;
        } else {
          T->tab[i][j] = ROCHER;
        }
